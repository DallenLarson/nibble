...
- 1.1.2 in the project file, the app manifest, the menu and the page footer.
...
- [ ] **Sign the binaries.** `Nibble.exe`, the `Nibble-*-Setup.exe` and the `unins000.exe` the
...
- [x] **Put the download in the README.** The Install section links
      `https://github.com/DallenLarson/nibble/releases/latest`, and the latest release carries
      the installer and the portable `Nibble.exe`.
...
- [x] **Publish the repository.** `DallenLarson/nibble`, public, MIT, tagged `v1.1.0`.
...
- [x] **Add an updater, or say there is none.** There is one from 1.1.0: it reads this
      repository's latest release, downloads that release's installer into
      `%AppData%\Nibble\updates`, and applies it at the next launch (`Updater.cs`,
      `--check-updates`, `tools/UpdateTest.ps1`). Proven end to end against a locally built
      "9.9.9" release: downloaded, applied on the next start, and the browser came back on it.
      Installs older than 1.0.2 have no updater in them and need one manual download.
- [ ] **Only ship updates people can trust.** The updater runs an installer it downloaded, and
      unsigned it cannot prove that installer is Nibble's — the trust is GitHub over TLS plus
      the size the feed reports. Signing the binaries (the first item) closes that, and is worth
      doing *before* advertising the auto-update.
